...
# lconvert.
$EXTRACT_TR_STRINGS `find activities/ core/ -name \*.cpp -o -name \*.h -o -name \*.qml -o -name \*.js` -o $podir/gcompris_qt.pot

$EXTRACT_TR_STRINGS `find server/ -name \*.cpp -o -name \*.h -o -name \*.qml -o -name \*.js` -o $podir/gcompris_server.pot

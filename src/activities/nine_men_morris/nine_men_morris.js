...
function secondPhase() {
	//items.firstInitialVisible = false
    //items.secondInitialVisible = false
...
function checkGameWon() {
	...
	if (((numberOfSecondPieces < 3 && !items.playSecond) || (numberOfFirstPieces < 3 && items.playSecond)) ||  
		...
		items.instructionTxt = "Congratulation"
		items.bonus.good("flower")
		if(twoPlayer) {
			items.instructionTxt = "Congratulation Player 1"
		...
		if(twoPlayer) {
			items.bonus.good("flower")
			items.instructionTxt = "Congratulation Player 2"
			items.bonus.isWin = false
		}
		else {
			items.instructionTxt = "Try Again"
			items.bonus.bad("tux")
		}
	}
	else {

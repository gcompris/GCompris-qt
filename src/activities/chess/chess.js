...
/* Hacked function below */

function simplifiedStateHACK(state) {
    for(var i = state.length - 1; i >= 0; --i) {
        if(state[i] !== 16) {
            switch(state[i]) {
                case 3:
...
//HACK Removing black king causes subsequent moves to need to be double clicked , so keeping king on for now return 1
                case 13:
...
//HACKBELOW
if (simplifiedStateHACK(state['board']) == 0){
...
//HACK    if(move.ok) {
        ...
        clearAcceptMove()
        items.redo_stack = []
//HACK        if(!items.twoPlayer)
//HACK            items.trigComputerMove.start()

        items.from = -1;
//HACK    } else {
        // Probably a check makes the move is invalid
//HACK        updateMessage(move)
//HACK    }
    return move.ok
}
...
//HACK                Engine.p4_unmake_move(state, move);
                continue;
            }

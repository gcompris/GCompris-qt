...
function valuesToSubtraction() {
    if (items.operation > 0) {    // subtraction
...
function randomValues() {
    ...
    for (var i = 0; i < items.nbLines; i++) {
        ...
        operators.push((items.operation === 0) ? "+" : "-")
    }
    valuesToSubtraction()
}
        ...
        if (ok) {
            ...
            for (i = 0; i < items.numbersModel.count; i++) {    // Build the new numberValue (from dropped numbers)
                ...
                if (items.numberRepeater.itemAt(i).numberValue !== "0")
                    result += (((i === 0) || (items.operation === 0)) ? 1 : -1) * value
            }
            items.board.result = result
            if (result < 0)
                ok = false
//            console.warn(result)
    ...
    if (ok) {
        ...
        if (!level.doItYourself)
            items.caption.text = qsTr("Solve:") + " " + operationString
...
function handleEvents(event) {
    var str = ""
    if (event.modifiers & Qt.AltModifier ) {    // Keys for debugging tools
        switch (event.key) {
        case Qt.Key_Return:

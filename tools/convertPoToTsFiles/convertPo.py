...
if len(sys.argv) < 4:
    print('Usage : python convertPo.py gcompris_gtk.po gcompris_qt_template.ts $LOCALE. Output will be gcompris_$LOCALE.ts ($LOCALE is fr_FR for example).')
    sys.exit(1)
...
for messages in root.iter('message'):
    ...
    if original != None:
      for entry in po:
        if entry.msgid.encode('utf-8') == original.encode('utf-8'):
